...
const Card: React.FC<CardProps> = ({ AnimeID, data, player = null, BottomText = null, deletion }) => {
  ...
  return (
    <div className="card-container" title={data.title} ref={cardRef} onClick={ContinueWatch}>
      ...
      {BottomText && <div className="card-continue-watch-text">{BottomText}</div>}
      {deletion && <div className="card-delete-icon material-symbols-outlined" onClick={(event) => deletion(event, AnimeID)}>close</div>}
    </div>
  )
}
